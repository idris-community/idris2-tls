...
ReadHack t_ok t_read_failed True = Res (List Bits8) (\_ => t_ok)
...
record Handle (t_ok : Type) (t_closed : Type) (t_read_failed : Type) (t_write_failed : Type) where
  ...
  1 underlying : t_ok
  do_read : forall m. LinearIO m => (1 _ : t_ok) -> (len : Int) -> L1 m $ Res Bool $ ReadHack t_ok t_read_failed
...
public export
read : LinearIO m => (1 _ : Handle t_ok t_closed t_read_failed t_write_failed) -> (len : Int) -> L1 m $ Res Bool $ \case
  False => t_read_failed
...
{-
socket_to_handle : (1 _ : Socket Open) -> Handle (Socket Open) (Socket Closed) (Res SocketError $ \_ => Socket Closed) (Res Int $ \_ => Socket Closed)
